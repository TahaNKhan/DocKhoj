...
> p2-T01 – p2-T20 are the spec's original task list. p2-T21 – p2-T26 are post-spec priority tasks that landed as follow-ups after the spec was frozen (markdown rendering, scroll/layout fixes, infra iteration loop, persistence overhaul, auto-scroll on session load). All are `done`.

---

## p2-T01 — Add new dependencies
...
- **Maps to FR:** FR-22 (deps allowlist), p2-T01 setup
...
## p2-T03 — Extract design tokens + base styles
...
## p2-T04 — Build static UI scaffold
...
## p2-T05 — SQLite layer + migration runner
...
## p2-T08 — Path migration cut (existing endpoints → /api/*)
...
## p2-T09 — Wire SPA to /api/sessions
...
- **Depends on:** p2-T07, p2-T04
- **Estimate:** M
- **Status:** done

## p2-T10 — OpenAI streaming wrapper (additive) + title generator service
...
## p2-T12 — SSE route + client SSE parser + post-`done` title event
...
- **Depends on:** p2-T11, p2-T09, p2-T10
- **Estimate:** L
- **Status:** done

## p2-T13 — /api/status route + TopBar wiring
...
## p2-T14 — Upload progress via XHR (no SSE) — historical marker

- **Description:** Adopt the simpler primitive for upload progress: `XMLHttpRequest.upload.onprogress` reports transport progress (bytes flowing to the server) natively in the browser; the final `success` / `failed` status comes back in the same POST `/api/upload` response. No server-side `EventEmitter`, no `GET /api/upload/progress` SSE channel, no race-on-subscribe semantics. The earlier p2-T14 SSE design was over-engineered for a single-user self-hosted app where uploads are infrequent (see `design.md` §Upload progress for the rationale). This task is folded into **p2-T15** below; the p2-T14 line is kept only as a historical marker.
- **Maps to FR:** FR-25, FR-26, FR-27
- **Maps to design:** §Key algorithms > Upload progress
- **Acceptance:** Documented in p2-T15.
- **Depends on:** —
- **Estimate:** —
- **Status:** done

## p2-T15 — Wire upload UI to live progress
...
## p2-T17 — SPA fallback + Dockerfile + docker-compose
...
- **Depends on:** p2-T08, p2-T02
- **Estimate:** M
- **Status:** done

## p2-T18 — Root package.json scripts
...
## p2-T19 — SPA fallback route tests
...
- **Depends on:** p2-T08, p2-T12, p2-T17, p2-T19
- **Estimate:** M
- **Status:** done

## p2-T21 — Markdown rendering in chat assistant bubbles (PRIORITY)
...
- **Depends on:** p2-T12 (streaming pipeline already produces text)
- **Estimate:** S
- **Status:** done

## p2-T22 — Chat scroll container (PRIORITY)
...
## p2-T23 — Chat layout refactor + composer pinned to bottom
...
- **Depends on:** p2-T04 (sidebar/composer static scaffold)
- **Estimate:** M
- **Status:** done

## p2-T24 — restart.sh hot-iteration loop
...
## p2-T25 — Persist Qdrant + SQLite to ~/.dockhoj
...
## p2-T26 — Auto-scroll chat to bottom on session load
...
- p2-T14 is a historical marker only — the SSE-based upload progress design was rolled into p2-T15's XHR-based approach.
- T40 (component tests) and T41 (e2e tests) were dropped per commit `ab06815 chore(tasks): drop T40 (component tests) + T41 (e2e); add p2-T21 + p2-T22 UI priorities`. Their coverage intent is captured by the route tests + the per-component tests written alongside p2-T04-p2-T16.
- p2-T21-p2-T26 are post-spec priority tasks added after the original spec was frozen. They're not in the spec README's "p2-T01 … p2-T20" pointer range — this is the canonical record of what shipped under Phase 02.