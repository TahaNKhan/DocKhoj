...
- **Description.** Update `services/qdrant.ts`:
  ...
  - Update `ensurePayloadIndexes()` to add `ownerId` and `visibility` indexes (per FR-33).
...
- **Acceptance criteria.**
  ...
  - Payload index check: query `qdrantClient.getCollection(COLLECTION_NAME).payload_schema` shows `ownerId: keyword, visibility: keyword`.
- **Dependencies.** T3.
- **Estimate.** M.
- **Status.** todo.
